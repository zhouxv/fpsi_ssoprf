...
# Prefix-fmap experiment script.
#
# Scalar syntax: -i <matching-points> -try <count>
# List syntax:   -nn <values...> -d <values...> -delta <values...>
# A list ends when the next token beginning with '-' is encountered.
# Example: ./bench_fmap_prefix.sh -nn 8 12 -d 2 6 -delta 10 60 -i 7
#
...
# Protocol 2 is fixed, and the timestamped CSV is always created beside this
# script rather than at a user-provided path.
...
Defaults: protocol=2 (fmap-prefix), nn=(8 12 16), d=(2 6 10 15),
...
output_file="${SCRIPT_DIR}/fmap_prefix_results_$(date +%Y%m%d_%H%M%S).csv"
...
for nn in "${ns[@]}"; do for dim in "${dims[@]}"; do
  for delta in "${deltas[@]}"; do run_case "${nn}" "${dim}" "${delta}"; done
done; done
